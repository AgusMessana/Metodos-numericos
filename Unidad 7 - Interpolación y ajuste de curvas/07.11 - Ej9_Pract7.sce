...
function w = er(x,n)
    nodos = -5:10/n:5 // nodos = x
    y = 1./(1.+nodos.^2) // calculo de y. Se evalúa cada valor de nodo en 1/(1+x^2)
    p = DD_Newton(nodos,y) // polinomio interpolante
    w = 1./(1.+x.^2) - horner(p,x) //error
endfunction
